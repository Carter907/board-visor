...
function Board({team, onSquareClicked, coordsVisible}: BoardProps) {
    ...
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            if ((i + j) % 2 == 0) {
                squares.push(
                    <div className="shadow-inner size-[60px] md:size-28 bg-stone-200 flex flex-col justify-center" onClick={() => {onSquareClicked(i, j)}}>
                ...
                squares.push(
                    <div className="shadow-inner size-[60px] md:size-28 bg-neutral-700 flex flex-col justify-center" onClick={() => {onSquareClicked(i, j)}}>
